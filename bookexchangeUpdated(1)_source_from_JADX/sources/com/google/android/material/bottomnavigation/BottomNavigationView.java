...
public class BottomNavigationView extends FrameLayout {
    private static final int DEF_STYLE_RES = C1130R.style.Widget_Design_BottomNavigationView;
    ...
    public BottomNavigationView(Context context, AttributeSet attrs) {
        this(context, attrs, C1130R.attr.bottomNavigationStyle);
    }
    ...
    public BottomNavigationView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        TintTypedArray a = ThemeEnforcement.obtainTintedStyledAttributes(context2, attrs, C1130R.styleable.BottomNavigationView, defStyleAttr, C1130R.style.Widget_Design_BottomNavigationView, C1130R.styleable.BottomNavigationView_itemTextAppearanceInactive, C1130R.styleable.BottomNavigationView_itemTextAppearanceActive);
        if (a.hasValue(C1130R.styleable.BottomNavigationView_itemIconTint)) {
            this.menuView.setIconTintList(a.getColorStateList(C1130R.styleable.BottomNavigationView_itemIconTint));
        } else {
            BottomNavigationMenuView bottomNavigationMenuView = this.menuView;
            bottomNavigationMenuView.setIconTintList(bottomNavigationMenuView.createDefaultColorStateList(16842808));
        }
        setItemIconSize(a.getDimensionPixelSize(C1130R.styleable.BottomNavigationView_itemIconSize, getResources().getDimensionPixelSize(C1130R.dimen.design_bottom_navigation_icon_size)));
        if (a.hasValue(C1130R.styleable.BottomNavigationView_itemTextAppearanceInactive)) {
            setItemTextAppearanceInactive(a.getResourceId(C1130R.styleable.BottomNavigationView_itemTextAppearanceInactive, 0));
        }
        if (a.hasValue(C1130R.styleable.BottomNavigationView_itemTextAppearanceActive)) {
            setItemTextAppearanceActive(a.getResourceId(C1130R.styleable.BottomNavigationView_itemTextAppearanceActive, 0));
        }
        if (a.hasValue(C1130R.styleable.BottomNavigationView_itemTextColor)) {
            setItemTextColor(a.getColorStateList(C1130R.styleable.BottomNavigationView_itemTextColor));
        }
        if (getBackground() == null || (getBackground() instanceof ColorDrawable)) {
            ViewCompat.setBackground(this, createMaterialShapeDrawableBackground(context2));
        }
        if (a.hasValue(C1130R.styleable.BottomNavigationView_elevation)) {
            ViewCompat.setElevation(this, (float) a.getDimensionPixelSize(C1130R.styleable.BottomNavigationView_elevation, 0));
        }
        DrawableCompat.setTintList(getBackground().mutate(), MaterialResources.getColorStateList(context2, a, C1130R.styleable.BottomNavigationView_backgroundTint));
        setLabelVisibilityMode(a.getInteger(C1130R.styleable.BottomNavigationView_labelVisibilityMode, -1));
        setItemHorizontalTranslationEnabled(a.getBoolean(C1130R.styleable.BottomNavigationView_itemHorizontalTranslationEnabled, true));
        int itemBackground = a.getResourceId(C1130R.styleable.BottomNavigationView_itemBackground, 0);
        if (itemBackground != 0) {
            this.menuView.setItemBackgroundRes(itemBackground);
        } else {
            setItemRippleColor(MaterialResources.getColorStateList(context2, a, C1130R.styleable.BottomNavigationView_itemRippleColor));
        }
        if (a.hasValue(C1130R.styleable.BottomNavigationView_menu)) {
            inflateMenu(a.getResourceId(C1130R.styleable.BottomNavigationView_menu, 0));
        }
        a.recycle();
    ...
    private void addCompatibilityTopDivider(Context context) {
        ...
        divider.setBackgroundColor(ContextCompat.getColor(context, C1130R.C1131color.design_bottom_navigation_shadow_color));
        divider.setLayoutParams(new FrameLayout.LayoutParams(-1, getResources().getDimensionPixelSize(C1130R.dimen.design_bottom_navigation_shadow_height)));
        addView(divider);
    }
