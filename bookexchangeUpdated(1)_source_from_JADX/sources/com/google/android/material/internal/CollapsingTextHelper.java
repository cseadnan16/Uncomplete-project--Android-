...
public final class CollapsingTextHelper {
    ...
    private void calculateBaseOffsets() {
        ...
        float textLayoutHeight = staticLayout2 != null ? (float) staticLayout2.getHeight() : 0.0f;
        ...
        if (i == 48) {
            ...
            if (shouldDrawMultiline()) {
                f2 = ((float) this.collapsedBounds.centerY()) - (textHeight / 2.0f);
            } else {
                f2 = ((float) this.collapsedBounds.centerY()) + textOffset;
            }
        ...
        float width2 = (staticLayout3 == null || this.maxLines <= 1 || this.isRtl) ? measuredWidth : staticLayout3.getLineWidth(0);
        ...
        if (i3 == 48) {
            ...
            float textOffset2 = ((this.textPaint.descent() - this.textPaint.ascent()) / 2.0f) - this.textPaint.descent();
            if (shouldDrawMultiline()) {
                f = ((float) this.expandedBounds.centerY()) - (textLayoutHeight / 2.0f);
            } else {
                f = ((float) this.expandedBounds.centerY()) + textOffset2;
            }
            this.expandedDrawY = f;
        } else {
            if (shouldDrawMultiline()) {
                offset = textLayoutHeight - this.textPaint.descent();
            }
            this.expandedDrawY = ((float) this.expandedBounds.bottom) - offset;
        }
    ...
    public void draw(Canvas canvas) {
        ...
        if (this.textToDraw != null && this.drawTitle) {
            ...
            float ascent = (float) this.textLayout.getLineAscent(0);
            float f = this.scale;
            if (f != 1.0f) {
                canvas.scale(f, f, x, y);
            }
            if (drawTexture) {
                canvas.drawBitmap(this.expandedTitleTexture, x, y + ascent, this.texturePaint);
    ...
    private void drawMultinlineTransition(Canvas canvas, float currentExpandedX, float x, float y, float ascent) {
        String tmp;
        Canvas canvas2 = canvas;
        float f = currentExpandedX;
        float f2 = ascent;
        int originalAlpha = this.textPaint.getAlpha();
        canvas2.translate(f, y);
        this.textPaint.setAlpha((int) (this.expandedTextBlend * ((float) originalAlpha)));
        this.textLayout.draw(canvas2);
        canvas2.translate(x - f, 0.0f);
        this.textPaint.setAlpha((int) (this.collapsedTextBlend * ((float) originalAlpha)));
        CharSequence charSequence = this.textToDrawCollapsed;
        canvas.drawText(charSequence, 0, charSequence.length(), 0.0f, (-f2) / this.scale, this.textPaint);
        ...
        canvas.drawText(tmp, 0, Math.min(this.textLayout.getLineEnd(0), tmp.length()), 0.0f, (-f2) / this.scale, this.textPaint);
    }
