...
public final class DefaultImageHeaderParser implements ImageHeaderParser {
    ...
    private static final class StreamReader implements Reader {
        ...
        /* renamed from: is */
        private final InputStream f55is;
        ...
        public int getUInt16() throws IOException {
            return ((this.f55is.read() << 8) & MotionEventCompat.ACTION_POINTER_INDEX_MASK) | (this.f55is.read() & 255);
        }

        public short getUInt8() throws IOException {
            return (short) (this.f55is.read() & 255);
        }
        ...
        public long skip(long total) throws IOException {
            ...
            long toSkip = total;
            while (toSkip > 0) {
                long skipped = this.f55is.skip(toSkip);
                if (skipped > 0) {
                    toSkip -= skipped;
                } else if (this.f55is.read() == -1) {
                    break;
                } else {
                    toSkip--;
                }
        ...
        public int read(byte[] buffer, int byteCount) throws IOException {
            ...
            while (toRead > 0) {
                int read = this.f55is.read(buffer, byteCount - toRead, toRead);
                int read2 = read;
                if (read == -1) {
                    break;
                }
