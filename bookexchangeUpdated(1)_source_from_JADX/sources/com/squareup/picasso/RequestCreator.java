...
public class RequestCreator {
    ...
    public Bitmap get() throws IOException {
        ...
        if (this.deferred) {
            ...
            Action action = new GetAction(this.picasso, request, this.memoryPolicy, this.networkPolicy, this.tag, C0967Utils.createKey(finalData, new StringBuilder()));
    ...
    public void fetch(Callback callback) {
        ...
        if (this.deferred) {
            ...
            String key = C0967Utils.createKey(request, new StringBuilder());
            ...
            if (this.picasso.loggingEnabled) {
                ...
                C0967Utils.log("Main", "completed", plainId, "from " + Picasso.LoadedFrom.MEMORY);
            }
            if (callback != null) {
                callback.onSuccess();
            }
    ...
    public void into(Target target) {
        ...
        if (target2 == null) {
            ...
            Request request = createRequest(started);
            String requestKey = C0967Utils.createKey(request);
    ...
    public void into(RemoteViews remoteViews, int viewId, int[] appWidgetIds, Callback callback) {
        ...
        if (remoteViews == null) {
            ...
            Request request = createRequest(started);
            performRemoteViewInto(new RemoteViewsAction.AppWidgetAction(this.picasso, request, remoteViews, viewId, appWidgetIds, this.memoryPolicy, this.networkPolicy, C0967Utils.createKey(request, new StringBuilder()), this.tag, this.errorResId, callback));
        } else {
            throw new IllegalArgumentException("Cannot use placeholder or error drawables with remote views.");
        }
    }
    ...
    private Request createRequest(long started) {
        ...
        if (loggingEnabled) {
            C0967Utils.log("Main", "created", request.plainId(), request.toString());
        }
        Request transformed = this.picasso.transformRequest(request);
        if (transformed != request) {
            transformed.f82id = id;
            transformed.started = started;
            if (loggingEnabled) {
                String logId = transformed.logId();
                C0967Utils.log("Main", "changed", logId, "into " + transformed);
            }
        }
        return transformed;
    }
