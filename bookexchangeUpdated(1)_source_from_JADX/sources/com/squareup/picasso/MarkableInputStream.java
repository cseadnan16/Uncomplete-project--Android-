...
final class MarkableInputStream extends InputStream {
    ...
    private MarkableInputStream(InputStream in, int size, int limitIncrement2) {
        ...
        this.f81in = !in.markSupported() ? new BufferedInputStream(in, size) : in;
    ...
    private void setLimit(long limit2) {
        try {
            if (this.reset >= this.offset || this.offset > this.limit) {
                ...
                this.f81in.mark((int) (limit2 - this.offset));
            } else {
                this.f81in.reset();
                this.f81in.mark((int) (limit2 - this.reset));
    ...
    private void skip(long current, long target) throws IOException {
        while (current < target) {
            long skipped = this.f81in.skip(target - current);
            if (skipped == 0) {
                if (read() != -1) {
                    skipped = 1;
                } else {
    ...
    public int read(byte[] buffer) throws IOException {
        ...
        int count = this.f81in.read(buffer);
        if (count != -1) {
    ...
    public int read(byte[] buffer, int offset2, int length) throws IOException {
        ...
        int count = this.f81in.read(buffer, offset2, length);
        if (count != -1) {
            this.offset += (long) count;
        }
        return count;
    ...
    public long skip(long byteCount) throws IOException {
        ...
        long skipped = this.f81in.skip(byteCount);
        this.offset += skipped;
        return skipped;
    }
