...
class BitmapHunter implements Runnable {
    ...
    static Bitmap decodeStream(Source source, Request request) throws IOException {
        ...
        boolean isWebPFile = C0967Utils.isWebPFile(bufferedSource);
    ...
    public void run() {
        try {
            ...
            if (this.picasso.loggingEnabled) {
                C0967Utils.log("Hunter", "executing", C0967Utils.getLogIdsForHunter(this));
            }
            Bitmap hunt = hunt();
            this.result = hunt;
            if (hunt == null) {
    ...
    public Bitmap hunt() throws IOException {
        ...
        if (!MemoryPolicy.shouldReadFromMemoryCache(this.memoryPolicy) || (bitmap = this.cache.get(this.key)) == null) {
            ...
            if (bitmap != null) {
                if (this.picasso.loggingEnabled) {
                    C0967Utils.log("Hunter", "decoded", this.data.logId());
                }
                ...
                if (this.data.needsTransformation() || this.exifOrientation != 0) {
                    synchronized (DECODE_LOCK) {
                        if (this.data.needsMatrixTransform() || this.exifOrientation != 0) {
                            ...
                            if (this.picasso.loggingEnabled) {
                                C0967Utils.log("Hunter", "transformed", this.data.logId());
                            }
                        }
                        if (this.data.hasCustomTransformations()) {
                            bitmap = applyCustomTransformations(this.data.transformations, bitmap);
                            if (this.picasso.loggingEnabled) {
                                C0967Utils.log("Hunter", "transformed", this.data.logId(), "from custom transformations");
                            }
                        }
                    }
                    if (bitmap != null) {
        ...
        if (this.picasso.loggingEnabled) {
            C0967Utils.log("Hunter", "decoded", this.data.logId(), "from cache");
        }
        return bitmap;
    }
        ...
        if (this.action == null) {
            ...
            if (loggingEnabled) {
                ...
                if (list == null || list.isEmpty()) {
                    C0967Utils.log("Hunter", "joined", request.logId(), "to empty hunter");
                } else {
                    C0967Utils.log("Hunter", "joined", request.logId(), C0967Utils.getLogIdsForHunter(this, "to "));
                }
            }
        } else {
    ...
    public void detach(Action action2) {
        ...
        if (this.picasso.loggingEnabled) {
            C0967Utils.log("Hunter", "removed", action2.request.logId(), C0967Utils.getLogIdsForHunter(this, "from "));
        }
    }
