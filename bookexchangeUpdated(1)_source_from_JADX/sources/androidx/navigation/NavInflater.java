...
public final class NavInflater {
            ...
            if (depth < innerDepth && type == 3) {
                ...
                if (TAG_ARGUMENT.equals(name)) {
                    ...
                    if (TAG_DEEP_LINK.equals(name)) {
                        ...
                        TypedArray a = resources.obtainAttributes(attributeSet, C1061R.styleable.NavInclude);
                        ((NavGraph) dest).addDestination(inflate(a.getResourceId(C1061R.styleable.NavInclude_graph, 0)));
                        a.recycle();
    ...
    private void inflateArgumentForDestination(Resources res, NavDestination dest, AttributeSet attrs, int graphResId) throws XmlPullParserException {
        TypedArray a = res.obtainAttributes(attrs, C1061R.styleable.NavArgument);
        String name = a.getString(C1061R.styleable.NavArgument_android_name);
        if (name != null) {
    ...
    private NavArgument inflateArgument(TypedArray a, Resources res, int graphResId) throws XmlPullParserException {
        ...
        boolean z = false;
        argumentBuilder.setIsNullable(a.getBoolean(C1061R.styleable.NavArgument_nullable, false));
        ...
        String argType = a.getString(C1061R.styleable.NavArgument_argType);
        if (argType != null) {
            navType = NavType.fromArgType(argType, res.getResourcePackageName(graphResId));
        }
        if (a.getValue(C1061R.styleable.NavArgument_android_defaultValue, value)) {
            if (navType == NavType.ReferenceType) {
                ...
                defaultValue = a.getString(C1061R.styleable.NavArgument_android_defaultValue);
            } else {
                int i = value.type;
                if (i == 3) {
    ...
    private void inflateDeepLink(Resources res, NavDestination dest, AttributeSet attrs) {
        TypedArray a = res.obtainAttributes(attrs, C1061R.styleable.NavDeepLink);
        String uri = a.getString(C1061R.styleable.NavDeepLink_uri);
    ...
    private void inflateAction(Resources res, NavDestination dest, AttributeSet attrs, XmlResourceParser parser, int graphResId) throws IOException, XmlPullParserException {
        Resources resources = res;
        AttributeSet attributeSet = attrs;
        TypedArray a = resources.obtainAttributes(attributeSet, C1061R.styleable.NavAction);
        int id = a.getResourceId(C1061R.styleable.NavAction_android_id, 0);
        NavAction action = new NavAction(a.getResourceId(C1061R.styleable.NavAction_destination, 0));
        NavOptions.Builder builder = new NavOptions.Builder();
        builder.setLaunchSingleTop(a.getBoolean(C1061R.styleable.NavAction_launchSingleTop, false));
        builder.setPopUpTo(a.getResourceId(C1061R.styleable.NavAction_popUpTo, -1), a.getBoolean(C1061R.styleable.NavAction_popUpToInclusive, false));
        builder.setEnterAnim(a.getResourceId(C1061R.styleable.NavAction_enterAnim, -1));
        builder.setExitAnim(a.getResourceId(C1061R.styleable.NavAction_exitAnim, -1));
        builder.setPopEnterAnim(a.getResourceId(C1061R.styleable.NavAction_popEnterAnim, -1));
        builder.setPopExitAnim(a.getResourceId(C1061R.styleable.NavAction_popExitAnim, -1));
