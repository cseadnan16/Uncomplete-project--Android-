...
final class BackStackRecord extends FragmentTransaction implements FragmentManager.BackStackEntry, FragmentManager.OpGenerator {
    ...
    public void dump(String prefix, PrintWriter writer, boolean full) {
        ...
        if (!this.mOps.isEmpty()) {
            ...
            for (int opNum = 0; opNum < numOps; opNum++) {
                FragmentTransaction.C0218Op op = (FragmentTransaction.C0218Op) this.mOps.get(opNum);
                switch (op.mCmd) {
                    case 0:
                        cmdStr = "NULL";
                        break;
            ...
            if (!(container == 0 || container == lastContainer)) {
                ...
                for (int i = startIndex; i < endIndex; i++) {
                    ...
                    for (int thoseOpIndex = 0; thoseOpIndex < numThoseOps; thoseOpIndex++) {
                        FragmentTransaction.C0218Op thatOp = (FragmentTransaction.C0218Op) record.mOps.get(thoseOpIndex);
        ...
        for (int opNum = 0; opNum < numOps; opNum++) {
            ...
            switch (op.mCmd) {
                ...
                case 8:
                    ...
                    break;
                case 9:
                    this.mManager.setPrimaryNavigationFragment((C0198Fragment) null);
                    break;
                case 10:
    ...
    /* access modifiers changed from: package-private */
    public C0198Fragment expandOps(ArrayList<C0198Fragment> added, C0198Fragment oldPrimaryNav) {
        int opNum = 0;
            ...
            if (i != 1) {
                if (i == 2) {
                    ...
                    for (int i2 = added.size() - 1; i2 >= 0; i2--) {
                        C0198Fragment old = added.get(i2);
                        if (old.mContainerId == containerId) {
                            if (old == f) {
                                alreadyAdded = true;
                            } else {
                                if (old == oldPrimaryNav) {
                                    this.mOps.add(opNum, new FragmentTransaction.C0218Op(9, old));
                                    opNum++;
                                    oldPrimaryNav = null;
                                }
                                FragmentTransaction.C0218Op removeOp = new FragmentTransaction.C0218Op(3, old);
                    ...
                    if (op.mFragment == oldPrimaryNav) {
                        this.mOps.add(opNum, new FragmentTransaction.C0218Op(9, op.mFragment));
                        opNum++;
                        oldPrimaryNav = null;
                    }
                } else if (i != 7) {
                    if (i == 8) {
                        this.mOps.add(opNum, new FragmentTransaction.C0218Op(9, oldPrimaryNav));
                        opNum++;
    ...
    /* access modifiers changed from: package-private */
    public C0198Fragment trackAddedFragmentsInPop(ArrayList<C0198Fragment> added, C0198Fragment oldPrimaryNav) {
        for (int opNum = this.mOps.size() - 1; opNum >= 0; opNum--) {
            FragmentTransaction.C0218Op op = (FragmentTransaction.C0218Op) this.mOps.get(opNum);
            int i = op.mCmd;
            if (i != 1) {
    ...
    public boolean isPostponed() {
        for (int opNum = 0; opNum < this.mOps.size(); opNum++) {
            if (isFragmentPostponed((FragmentTransaction.C0218Op) this.mOps.get(opNum))) {
                return true;
            }
        }
        return false;
    ...
    /* access modifiers changed from: package-private */
    public void setOnStartPostponedListener(C0198Fragment.OnStartEnterTransitionListener listener) {
        for (int opNum = 0; opNum < this.mOps.size(); opNum++) {
            FragmentTransaction.C0218Op op = (FragmentTransaction.C0218Op) this.mOps.get(opNum);
    ...
    private static boolean isFragmentPostponed(FragmentTransaction.C0218Op op) {
        C0198Fragment fragment = op.mFragment;
